...
Defaults to the ``develop`` environment, so developers can use ``python
...
if DJANGOENV == 'develop':  # Used for local development
    ...
    from vokss.develop.develop_settings import *
elif DJANGOENV == 'test':  # Used when running the Django tests locally
    from vokss.develop.test_settings import *
elif DJANGOENV == 'production':  # Used on production server
    from vokss.production.production_settings import *
elif DJANGOENV == 'staging':  # Used on staging server
    from vokss.production.staging_settings import *
else:
