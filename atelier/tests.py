...
class OrderModelTests(TestCase):
    ...
    def test_was_ordered_recently_with_future_order(self):
        """
        was_ordered_recently() returns False for order whose myorder_date
        is in the future.
        """
        time = timezone.now() + datetime.timedelta(days=30)
        future_order = MyOrder(myorder_date=time)
...
def test_was_ordered_recently_with_old_order(self):
    ...
    old_order = MyOrder(myorder_date=time)
    self.assertIs(old_order.was_ordered_recently(), False)

def test_was_ordered_recently_with_recent_order(self):
    """
    was_ordered_recently() returns True for order whose myorder_date
    is within the last day.
    """
    time = timezone.now() - datetime.timedelta(hours=23, minutes=59, seconds=59)
    recent_order = MyOrder(myorder_date=time)
