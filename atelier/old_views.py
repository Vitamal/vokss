...
def index(request):
    ...
    return render(request, 'atelier/product_list.html', context)


def detail(request, product_id):
    product = get_object_or_404(Product, pk=product_id)
    return render(request, 'atelier/detail.html', {'product': product})
...
def results(request, product_id):
    product = get_object_or_404(Product, pk=product_id)
    return render(request, 'atelier/results.html', {'product': product})